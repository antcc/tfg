...
\begin{definition}[Max-min composition] Let $\mathcal{R}_1$ and $\mathcal{R}_2$ be two fuzzy relations defined on $X \times Y$ and $Y \times Z$, respectively. The max-min composition of $\mathcal R_1$ and $\mathcal R_2$ is a fuzzy set $\mathcal R_1 \circ \mathcal R_2$ in $X \times Z$ defined by
\[
...
The Gaussian MF is arguably the most notable example of a composite MF, since it is a well known result that every $n-$dimensional Gaussian function is the product of $n$ one-dimensional Gaussian functions.
...
\begin{enumerate}
	...
	\item $G$ is a \textit{syntactic rule} which generates the terms in $T(x)$, and
...
\]
In particular, the operation of \textit{concentration} is defined as $CON(A)=A^2$, while that of \textit{dilation} is expressed by $DIL(A)=A^{0.5}$.

\end{definition}

Conventionally, we take $CON(A)$ and $DIL(A)$ to be the result of applying the modifiers \textit{very} and \textit{more or less}, respectively, to the linguistic term $A$. Following the definitions in the previous chapter, we can interpret the negation operator \textit{not} and the connectives \textit{and}, \textit{or} as follows:
\begin{align*}
	NOT(A) &= \neg A = \int_X 1 - \mu_A(x) \slash x,\\
	A \ AND \ B &= A \cap B = \int_X \mu_A(x) \land \mu_B(x) \slash x,\\
	A \ OR \ B &= A \cup B = \int_X \mu_A(x) \lor \mu_B(x) \slash x.
\end{align*}
